...
You can also run the CLI directly from Maven coordinates with jbx by passing the CLI main class explicitly:

[source,bash,subs='attributes']
----
jbx --main builders.dsl.spreadsheet.cli.SpreadsheetCli builders.dsl:spreadsheet-builder-cli:{project-version} create people.yml people.xlsx
jbx --main builders.dsl.spreadsheet.cli.SpreadsheetCli builders.dsl:spreadsheet-builder-cli:{project-version} query people.xlsx query.yml
----

Or with JBang, which can use the CLI JAR's `Main-Class` manifest entry:

[source,bash,subs='attributes']
----
jbang builders.dsl:spreadsheet-builder-cli:{project-version} create people.yml people.xlsx
jbang builders.dsl:spreadsheet-builder-cli:{project-version} query people.xlsx query.yml
----
...
=== 4.0.2
Added the `spreadsheet-builder-cli` module for creating workbooks from JSON/YAML data and querying Excel workbooks from serialized JSON/YAML criteria.

Published the query JSON Schema in the guide at link:schema/query.schema.json[`schema/query.schema.json`].

=== 4.0.1
Fixed Maven Central release artifact signing and kept the release workflow tag-only.

Removed retired Sheet.REST references from the documentation.

=== 4.0.0
Refreshed publishing and runtime dependencies, including Apache POI and Groovy-related dependencies.

Fixed Groovy DSL delegate keyword resolution and preserved static annotations in published metadata.

Refreshed Google credentials handling for CI and release workflows.
